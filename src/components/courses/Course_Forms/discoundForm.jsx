...
const formSchema = z.object({
    ...
    }).max(101,{message: "max discount is 100%"}),
});

export const DiscountForm = ({ initialData, courseId,setInitial }) => {
        ...
        <div className="mt-6 border bg-slate-100 rounded-md p-4">
            <div className="font-medium flex items-center justify-between">
                ...
                <Button onClick={toggleEdit} variant='ghost'>
                    {isEditing ? "Cancel" : <>
                        <Pencil className="h-4 w-4 mr-2"/> Edit discount
                    </>}
                </Button>
            </div>
            {!isEditing && (<p className="text-sm mt-2">{initialData?.discount?`$${initialData?.discount}%`:'No discount'}</p>)}
